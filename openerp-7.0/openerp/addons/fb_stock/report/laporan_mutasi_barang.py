...
class laporan_mutasi_barang(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(laporan_mutasi_barang, self).__init__(cr, uid, name, context=context)
        self.isi_laporan = []
        self.isi_saldo = []
    ...
    def get_nama_uom(self, form):
        ...
        self.cr.execute(" select d.product_id as id from product_template a \
            ...
            where  d.product_id is not null and a.categ_id = %s " % form['parent_id'][0])
    ...
    def get_default_code(self, form):
        ...
        self.cr.execute(" select d.product_id as id from product_template a \
            ...
            left join product_product c on a.id = c.product_tmpl_id \
            left join stock_move d on c.id = d.product_id \
            where  a.categ_id = %s and d.product_id is not null \
        " % form['parent_id'][0])
        cat = self.cr.dictfetchall()
        if i < len(cat):
            for ii in cat:
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(laporan_mutasi_barang, self).set_context(objects, data, ids, report_type=report_type)




report_sxw.report_sxw('report.laporan_mutasi_barang', 'stock.move', 'addons/fb_stock/report/laporan_mutasi_barang.rml', parser=laporan_mutasi_barang, header=False)
...
#         kriteria = [('date','>=',self.date_from),('date','<=',self.date_to)]
#
