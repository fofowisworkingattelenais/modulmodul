        ...
        for invoice in obj_inv.browse(cr, uid, context['active_ids'], context=context):
            ...
            if not taxform_ids and invoice.state in ('open','paid'):
                ...
                val_taxform ={
                    ...
                    'company_address' : invoice.company_id.company_tax_id and invoice.company_id.company_tax_id.partner_id.address[0].id or invoice.company_id.partner_id.address[0].id,
                    'company_npwp' : invoice.company_id.company_tax_id and invoice.company_id.company_tax_id.partner_id.npwp or invoice.company_id.partner_id.npwp,
                    'taxform_date' : date.today().strftime('%Y-%m-%d'),
                    'invoice_id' : invoice.id,
                    'partner_id' : invoice.partner_id.id,
                    'partner_address_id' : invoice.address_invoice_id.id,
                ...
                for invoice_line in invoice.invoice_line:
                    if invoice_line.price_subtotal > 0:
                        ...
                        taxes = obj_tax.compute_all(cr, uid, invoice_line.invoice_line_tax_id, price, invoice_line.quantity, product=invoice_line.product_id, address_id=invoice_line.invoice_id.address_invoice_id, partner=invoice_line.invoice_id.partner_id)
