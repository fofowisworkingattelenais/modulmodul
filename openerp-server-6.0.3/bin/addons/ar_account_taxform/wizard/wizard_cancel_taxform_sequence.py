...
class wizard_cancel_taxform_sequence(osv.osv_memory):
	...
	def cancel_sequence(self, cr, uid, ids, context=None):
		...
		for taxform in obj_taxform.browse(cr, uid, context['active_ids']):
			kriteria = [('taxform_id','=',taxform.id)]
			sequence_ids = obj_taxform_sequence.search(cr, uid, kriteria)
			
			if sequence_ids:
				obj_taxform_sequence.write(cr, uid, sequence_ids, {'taxform_id' : False})
				
			obj_taxform.write(cr, uid, [taxform.id], {'state':'cancel', 'name':'/'})		
