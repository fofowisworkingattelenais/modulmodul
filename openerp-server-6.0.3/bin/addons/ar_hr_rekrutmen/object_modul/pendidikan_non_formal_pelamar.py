...
class pendidikan_non_formal_pelamar(osv.osv):
	_name = 'hr.pendidikan_non_formal_pelamar'
	_description = 'Pendidikan Non Formal Pelamar'
	
	_columns = 	{
				'pelamar_id' : fields.many2one('hr.pelamar', 'Pelamar', required = True),
				'name' : fields.char('Pendidikan Non-Formal', size=100, required=True),
				'nama_institusi' : fields.char('Nama Institusi', size=100, required=True),
	...
	def create(self, cr, uid, values, context={}):
		...
		if situasi == 'aman':
			return super(pendidikan_non_formal_pelamar, self).create(cr, uid, values, context)
		else:
	...
	def copy(self, cr, uid, id, default=None, context={}):
		...
		if situasi == 'aman':
			return super(pendidikan_non_formal_pelamar, self).copy(cr, uid, id, default, context)
		else:
	...
	def unlink(self, cr, uid, ids, context={}):
		...
		if situasi == 'aman':
			return super(pendidikan_non_formal_pelamar, self).unlink(cr, uid, ids, context)
		else:
			raise osv.except_osv('Peringatan', 'Data tidak bisa dihapus')		
			

pendidikan_non_formal_pelamar()
