...
class rfq_external(report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context):
		super(rfq_external, self).__init__(cr, uid, name, context=context)
	...
	def user(self):
		user = self.pool.get('res.users').browse(self.cr, self.uid, [self.uid])[0]
		return user.name
...
report_sxw.report_sxw('report.purchase.rfq.external','purchase.order','addons/telenais_gdi/report/rfq_external.rml',parser=rfq_external, header=False)
