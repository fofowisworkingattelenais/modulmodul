...
class purchase_requisition(osv.osv):
	...
	_columns =	{
							...
							'warehouse_id': fields.many2one(obj='stock.warehouse', string='Warehouse', readonly=True, states={'draft':[('readonly',False)]}),
							'requisition_approval_ids' : fields.one2many(stirng='Purchase Requisition Approval', obj='titis.requisition_approval', fields_id='prr_id', readonly=True, states={'draft':[('readonly',False)]}),
							...
							'pr_type' : fields.selection(string='Purchase Requisition Type', selection=[('Normal','Normal'),('Urgent','Urgent'),('Top_Urgent','Top Urgent')], required=True, readonly=True, states={'draft':[('readonly',False)]}),
							'state': fields.selection([('draft','Draft'),('confirm','Confirm'),('waiting','Waiting For Approval'),('in_progress','In Progress'),('cancel','Cancelled'),('done','Done')], 'State', required=True)
							}
				...
				if requisition.requisition_approval_ids:
					for approval in requisition.requisition_approval_ids:
						if approval.approval_status == 'waiting':
							obj_approval.write(cr, uid, [approval.id], {'approval_status' : 'approved', 'approval_date' : datetime.today().strftime('%Y-%m-%d %H:%M:%S')})
		...
		for requisition in self.browse(cr, uid, ids):						
			...
			if self.check_approval(cr, uid, [requisition.id]):
				...
				if requisition.requisition_approval_ids:
					for approval in requisition.requisition_approval_ids:
						if approval.approval_status == 'waiting':
							obj_approval.write(cr, uid, [approval.id], {'approval_status' : 'bypass', 'user_bypass_id' : uid,  'approval_date' : datetime.today().strftime('%Y-%m-%d %H:%M:%S')})
