...
class loista_hutang_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(loista_hutang_report, self).__init__(cr, uid, name, context=context)
        self.isi_laporan = []
    ...
    def lines(self, form):
        ...
        if form['partner_id'] == '0':
            kriteria = [ ('date_invoice', '>=', form['from_date']), ('date_invoice', '<=' ,form['to_date']),
                        ('type', '=', 'in_invoice'), ('state', '=', 'open'), ('journal_id', '=',form['journal_id'])]
        else:
            kriteria = [('date_invoice', '>=', form['from_date']), ('date_invoice', '<=', form['to_date']),
                        ('type', '=', 'in_invoice'), ('state', '=', 'open'), ('journal_id', '=', form['journal_id']), ('partner_id', '=', form['partner_id']) ]
...
report_sxw.report_sxw('report.loista_hutang_report', 'account.invoice','addons/loista/report/loista_hutang_report.rml',parser=loista_hutang_report, header=False)
